...
# calculate_offset.sh
#
# Calculates the average estimated offset from the NMEA source
...
read -r total_sum count <<< $(awk '/NMEA/ {sum+=$5; count++} END {print sum, count}' "$logfile")

if [ "$count" -ne 0 ]; then
    avg_offset=$(bc -l <<< "$total_sum / $count")
    
    echo "Average estimated offset: $avg_offset"
    sudo sed -i "/refclock SHM 0 refid NMEA/ s/offset [0-9.]\+/offset $avg_offset/" "/etc/chrony/chrony.conf"
    ...
    echo "No NMEA records found. No changes made."
    exit 1 
fi