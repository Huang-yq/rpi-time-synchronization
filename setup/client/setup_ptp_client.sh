...
# setup_ptp_client.sh
# 
# Updates the PTP configuration to set the system as slave only, 
...
if grep -q "^masterOnly" "${PTP_CONF}"; then
    sudo sed -i "s/^slaveOnly.*/slaveOnly              1/" "${PTP_CONF}"
else
...
sudo sed -i "s/^ExecStart=.*/ExecStart=\/usr\/sbin\/phc2sys -w -s \/dev\/ptp0/" "${PHC2SYS_SERV}"
