...
# configure_chrony.sh
#
# Configures chrony with GPS and PPS sources and adjusts 
...
# Define the GPS and PPS configurations as variables
GPS_CONFIG="refclock SHM 0 refid GPS offset 0.000 precision 1e-3 poll 3 noselect"
PPS_CONFIG="refclock PPS /dev/pps0 refid PPS lock GPS poll 3"

# Check for the presence of NMEA configuration by looking for a constant part of it
if grep -q "refclock SHM 0 refid GPS" "/etc/chrony/chrony.conf"; then
    echo "Updating existing GPS source configuration..."
    sudo sed -i "/refclock SHM 0 refid GPS/c\\$GPS_CONFIG" "/etc/chrony/chrony.conf"
else
    echo "Adding GPS source configuration..."
    echo "$GPS_CONFIG" | sudo tee -a "/etc/chrony/chrony.conf" > /dev/null
fi
